...
export class ReplayJournal {
 ...
 async recent(head:string|undefined,world:World,limit=50,before?:string):Promise<{entries:JournalEntry[];hasMore:boolean}>{
  ...
  let collecting=!before;
  const entries:JournalEntry[]=[];let cursor:string|null=head,expected=digest(world);const seen=new Set<string>();
  while(cursor&&entries.length<limit){if(seen.size>=10000)throw Error('History page exceeds record search limit');if(seen.has(cursor))throw Error('Cyclic journal');seen.add(cursor);const record=await this.record(cursor);if(record.worldId!==world.id||record.hash!==expected)throw Error('Broken journal chain');if(collecting)entries.push({id:cursor,kind:record.kind,tick:record.tick,revision:record.revision,summary:record.kind==='step'?`Advanced ${record.days} day${record.days===1?'':'s'}`:record.kind==='proposal'?record.proposal.summary:`${record.reason} snapshot`});else if(cursor===before)collecting=true;expected=record.kind==='snapshot'?'':record.before;cursor=record.previous;if(cursor&&record.kind==='snapshot')expected=(await this.record(cursor)).hash;}
  if(!collecting)throw Error('History cursor is not committed');
  return {entries,hasMore:cursor!==null};
 }
 private async replayRecord(record:Record,prior:World|undefined,worldId:string):Promise<World> {
  let replay=prior;
  if(record.kind==='snapshot'){
   ...
   replay=validateWorld(migrateWorld(raw.world));
  }else{
   if(!replay||digest(replay)!==record.before)throw Error('Journal input state mismatch');
   if(record.kind==='proposal')replay=applyProposal(replay,record.proposal);
   else for(let i=0;i<record.days;i++){replay=advance(replay);if(i%10===0)await new Promise<void>(resolve=>setImmediate(resolve));}
  }
  if(!replay||replay.id!==worldId||digest(replay)!==record.hash||replay.tick!==record.tick||replay.revision!==record.revision)throw Error('Replay result differs from recorded state');
  return replay;
 }
 async reconstruct(head:string|undefined,world:World,target:string,options:{maxDays?:number;maxRecords?:number;checkpoint?:(hash:string)=>Promise<World|undefined>}={}) {
  Digest.parse(target);if(!head)throw Error('No replay history yet');await this.head(head,world);
  const maxDays=options.maxDays??1000,maxRecords=options.maxRecords??10000;
  if(!Number.isInteger(maxDays)||maxDays<0||maxDays>100000||!Number.isInteger(maxRecords)||maxRecords<1||maxRecords>10000)throw Error('Invalid reconstruction budget');
  const records:Record[]=[],seen=new Set<string>();let cursor:string|null=head,expected:string|undefined=digest(world),found=false,days=0,base:World|undefined,baseKind:'journal'|'checkpoint'='journal';
  while(cursor){
   if(seen.size>=maxRecords)throw Error('Historical branch exceeds record search limit');
   ...
   if(cursor===target)found=true;
   if(found){
    if(record.kind==='snapshot'){base=await this.replayRecord(record,undefined,world.id);break;}
    const checkpoint=await options.checkpoint?.(record.hash);
    if(checkpoint){
     base=validateWorld(checkpoint);
     if(base.id!==world.id||digest(base)!==record.hash||base.tick!==record.tick||base.revision!==record.revision)throw Error('Checkpoint does not match replay record');
     baseKind='checkpoint';break;
    }
    records.push(record);if(record.kind==='step')days+=record.days;
    if(days>maxDays)throw Error(`Historical branch needs more than ${maxDays} replay days; use a nearer checkpoint or snapshot`);
   }
   expected=record.kind==='snapshot'?undefined:record.before;cursor=record.previous;
  }
  if(!found)throw Error('Requested record is not in this world’s committed history');
  if(!base)throw Error('Historical branch has no verified starting snapshot');
 ...
 async verify(head:string|undefined,world:World,maxDays=10000,maxRecords=10000){
  ...
  for(const record of records.reverse()){
   replay=await this.replayRecord(record,replay,world.id);
   if(record.kind==='snapshot')snapshots++;else if(record.kind==='proposal')proposals++;
  }
