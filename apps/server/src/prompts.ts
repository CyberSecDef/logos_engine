...
export function forecast(world:World,proposal:Proposal) {
 ...
 for(const op of proposal.operations) {
  ...
  if(op.kind==='neighbor-visits-configure'||op.kind.startsWith('journey-')||op.kind==='resource-route-define'||op.kind==='resource-route-remove'||op.kind==='travel-permission'||op.kind==='food-trade-permission'||op.kind==='food-trade-configure'||op.kind==='soil-ecology-configure'||op.kind==='field-define'||op.kind==='field-remove')for(const tile of world.tiles)ids.add(tile.id);
