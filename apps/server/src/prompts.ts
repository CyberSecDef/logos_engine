import {factionSummary} from '../../../packages/engine/src/factions.js';
...
export class PromptService {
 ...
 private accept(world:World,job:PromptJob,raw:unknown) {
  ...
  for(const op of reply.operations) {
   ...
   if(op.kind==='conflict-configure'&&job.request.scope!=='world')throw Error('Conflict policy requires Entire world scope');
   if((op.kind==='faction-define'||op.kind==='faction-remove'||op.kind==='faction-relation'||op.kind==='faction-borders-configure')&&job.request.scope!=='world')throw Error('Faction registry changes require Entire world scope');
   if((op.kind==='knowledge-configure'||op.kind==='technology-configure'||op.kind==='technology-define')&&job.request.scope!=='world')throw Error('Technology definitions require Entire world scope');
   if((op.kind==='disease-configure'||op.kind==='disease-contact-configure')&&job.request.scope!=='world')throw Error('Disease configuration requires Entire world scope');
...
export function forecast(world:World,proposal:Proposal) {
 ...
 const applied=candidate;const conflictDays:NonNullable<NonNullable<World['conflict']>['lastDay']>[]=[];const migrationDays:NonNullable<NonNullable<World['migration']>['lastDay']>[]=[];let routePreview:NonNullable<World['resourceRoutes']>['lastDay'];let tradePreview:NonNullable<World['foodTrade']>['lastDay'];
 for(let i=0;i<5;i++){if(!baselineError)try{baseline=advance(baseline);}catch(error){if(!(error instanceof PluginExecutionError))throw error;baselineError=error.message;}candidate=advance(candidate);if(candidate.conflict?.lastDay)conflictDays.push(candidate.conflict.lastDay);if(candidate.migration?.enabled&&candidate.migration.lastDay)migrationDays.push(candidate.migration.lastDay);if(i===0)routePreview=candidate.resourceRoutes?.lastDay;if(i===0&&candidate.foodTrade?.enabled)tradePreview=candidate.foodTrade.lastDay;}
 ...
 for(const op of proposal.operations) {
  ...
  if(op.kind==='conflict-configure'||op.kind==='army-depart'||op.kind==='garrison-configure'||op.kind.startsWith('faction-')||op.kind==='knowledge-configure'||op.kind.startsWith('technology-')||op.kind.startsWith('research-')||op.kind.startsWith('disease-')||op.kind.startsWith('water-')||op.kind==='sanitation-configure'||op.kind.startsWith('air-')||op.kind==='migration-configure'||op.kind==='neighbor-visits-configure'||op.kind.startsWith('journey-')||op.kind==='resource-route-define'||op.kind==='resource-route-remove'||op.kind==='travel-permission'||op.kind==='food-trade-permission'||op.kind==='food-trade-configure'||op.kind==='soil-ecology-configure'||op.kind==='field-define'||op.kind==='field-remove')for(const tile of world.tiles)ids.add(tile.id);
