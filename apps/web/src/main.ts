...
function selectTile(id:number) {
 ...
 text('travel-status',`Route travel: ${t.travelAllowed===false?'closed':'open'}. Applies to named resource routes; people are not traveling yet.`);
...
type WorldSummary={name:string;tick:number;tiles:number;fields:number;plugins:number;rules:number;interventions:number;entities?:number;entityTypes?:number;settlements?:number;population?:number};
...
function reviewWorld(title:string,summary:WorldSummary,note:string,label:string,path:string,body:unknown) {
 ...
 text('world-review-summary',`${summary.name} · day ${summary.tick} · ${summary.tiles} zones · ${summary.settlements??0} settlements (${fmt(summary.population??0)} inhabitants) · ${summary.fields} custom properties · ${summary.rules} custom rules · ${summary.plugins} plugins · ${summary.entities??0} entities (${summary.entityTypes??0} types) · ${summary.interventions} recorded changes.`);
...
function describeOperation(o:Operation):string {
 switch(o.kind){
  ...
  case 'travel-permission':return `${o.allowed?'Open':'Close'} zone ${o.tileId} to named resource-route travel in both directions. Food sharing, communication and existing generic stock transfers keep their own settings; population travel is not implemented yet.`;
...
function promptState(id:string|null) {
 ...
 for(const name of ['chat-discuss','chat-propose','chat-export','chat-close','worlds-toggle','play','step','water-explain','travel-permission','route-create','food-trade-enable','food-trade-permission','soil-enable','settlement-create','settlement-food','rain-change','elevation-change','temperature-change','temperature-reset','communication-change'])$<HTMLButtonElement>(name).disabled=!!id;
