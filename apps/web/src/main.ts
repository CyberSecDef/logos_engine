...
function selectTile(id:number) {
 ...
 if(settlement?.lastDay?.waterQualityPermille!==undefined){const note=document.createElement('p');note.className='water-farming-report muted';note.textContent=`Farm water quality day ${settlement.lastDay.tick}: ${settlement.lastDay.waterQualityPermille/10}% multiplier after weather and soil. No illness simulated.`;container.append(note);}
