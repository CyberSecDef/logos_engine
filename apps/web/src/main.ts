...
function selectTile(id:number) {
 ...
 text('travel-status',`Route travel: ${t.travelAllowed===false?'closed':'open'}. Applies to named resource routes and migration journeys.`);
...
$('travel-permission').onclick=()=>void propose({kind:'travel-permission',tileId:selected,allowed:world.tiles[selected].travelAllowed===false},'Change this zone’s travel permission for named resource routes.');
...
function describeOperation(o:Operation):string {
 switch(o.kind){
  ...
  case 'travel-permission':return `${o.allowed?'Open':'Close'} zone ${o.tileId} to named resource-route travel in both directions. Food sharing, communication and existing generic stock transfers keep their own settings; travelers may wait if their next leg or arrival is blocked.`;
