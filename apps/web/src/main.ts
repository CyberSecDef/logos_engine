...
async function reviewProposal(proposal:Proposal) {
 const result=await api<{appearanceChanges:{total:number;tiles:{tileId:number;before:{label:string;color:string;asset:string;layers:{asset:string;opacity:number}[]};after:{label:string;color:string;asset:string;layers:{asset:string;opacity:number}[]}}[]};pluginMigrations:{id:string;label:string;totalTiles:number;tiles:{tileId:number;before:Record<string,number>;after:Record<string,number>}[]}[];tick:number;baselineError:string|null;baselineTick:number;totalTiles:number;resources:{fieldId:string;label:string;unit:string;baselineUnit:string|null;totalMilli:number;beforeMilli:number|null;baselineMilli:number|null}[];tiles:{properties:{id:string;label:string;unit:string;baselineUnit:string|null;after:number;baseline:number|null}[];id:number;after:World['tiles'][number];baseline:World['tiles'][number];waterMm:number;baselineWaterMm:number}[]}>('proposals/preview',proposal);
