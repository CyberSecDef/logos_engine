...
function selectTile(id:number) {
 ...
 text('disease-status',health?`Health ${world.disease?.enabled?'active':'paused'}: ${fmt(t.population-health.ill-health.immune)} susceptible, ${fmt(health.ill)} ill, ${fmt(health.immune)} immune. Active illness removes farm workers. No transmission or direct disease deaths in this first milestone.`:'Health tracking inactive. Activation starts everyone susceptible; cases can be introduced by the creator or polluted water. Contact transmission is the next milestone.');
 text('disease-toggle',world.disease?.enabled?'Review pausing health effects':'Review enabling health tracking');
 for(const name of ['disease-introduce','disease-treat'])$<HTMLButtonElement>(name).disabled=!!promptJobId||!world.disease;
...
$('disease-toggle').onclick=()=>void propose({kind:'disease-configure',tileId:selected,expectedVersion:world.disease?.version??0,enabled:!world.disease?.enabled,settings:{...(world.disease?.settings??DEFAULT_DISEASE_SETTINGS)}},'Configure health tracking, recovery, temporary immunity and illness workforce effects. No transmission yet.');
...
function describeOperation(o:Operation):string {
 switch(o.kind){
  ...
  case 'disease-configure':return `${o.enabled?'Enable/update':'Pause'} health effects worldwide, version ${o.expectedVersion} → ${o.expectedVersion+1}. Water exposure up to ${o.settings.waterExposurePermille/10}% of susceptible residents/day, scaled by standing-water quality. Recovery ${o.settings.recoveryPermille/10}% of ill per day; immunity loss ${o.settings.immunityLossPermille/10}% per day, minimum one when a rate is positive and people remain. Ill people cannot farm while active. Activation changes no cases immediately; polluted water can cause cases on subsequent ticks. No contact transmission or direct disease deaths yet; existing food shortages still apply.`;
