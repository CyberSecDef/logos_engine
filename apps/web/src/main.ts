...
async function reviewProposal(proposal:Proposal) {
 ...
 const operations=document.createElement('p');operations.textContent=proposal.operations.map(o=>o.kind==='rainfall'?`Zone ${o.tileId}: ${o.mmPerDay} mm rain/day`:o.kind==='elevation'?`Zone ${o.tileId}: ${o.deltaM>=0?'+':''}${o.deltaM} m elevation`:`Zone ${o.tileId}: communication ${o.enabled?'connected':'isolated'}`).join(' · ');container.append(operations);
 for(const tile of result.tiles){if(!proposal.operations.some(o=>o.tileId===tile.id)&&Math.abs(tile.waterMm-tile.baselineWaterMm)<0.1)continue;const p=document.createElement('p');p.textContent=`Day ${result.tick}, zone ${tile.id}: ${tile.waterMm.toFixed(1)} mm water (${tile.baselineWaterMm.toFixed(1)} mm without this change), ${tile.after.elevationM} m elevation.`;container.append(p);}
...
function promptState(id:string|null) {
 ...
 for(const name of ['chat-discuss','chat-propose','chat-export','chat-close','worlds-toggle','play','step','rain-change','elevation-change','communication-change'])$<HTMLButtonElement>(name).disabled=!!id;
