...
export function readValue(world:World,tileId:number,read:Read):number {
 const at=(id:number)=>read.source==='technologyKnown'?(world.tiles[id].research?.projects.some(p=>p.technologyId===read.technologyId&&p.completedTick!==undefined)?1:0):read.source==='technologyProgress'?(world.tiles[id].research?.projects.find(p=>p.technologyId===read.technologyId)?.progress??0):read.source==='waterPollution'?(world.tiles[id].waterQuality?.dissolved??0):read.source==='surfacePollution'?(world.tiles[id].waterQuality?.surface??0):read.source==='waterConcentration'?(world.tiles[id].waterL>0?(world.tiles[id].waterQuality?.dissolved??0)*1e6/world.tiles[id].waterL:0):read.source==='sanitationCondition'?(world.tiles[id].waterQuality?.sanitationCondition??0):read.source==='airLoad'?(world.tiles[id].air?.load??0):read.source==='airDensity'?(world.tiles[id].air?.load??0)/(world.cells[id].areaM2/1e6):read.source==='windBearingDeg'||read.source==='windPermille'?(world.tiles[id].air?.[read.source]??0):read.source==='foodRations'||read.source==='shortageDays'||read.source==='surplusDays'?(world.tiles[id].settlement?.[read.source]??0):read.source==='entity-count'||read.source==='entity-sum'?readEntities(world,id,read):read.source==='custom'?fieldValue(world,id,read.fieldId!):read.source==='waterMm'?world.tiles[id].waterL/world.cells[id].areaM2:world.tiles[id][read.source];
