...
export function advanceNeighborVisits(w:World):void {
 ...
 const jobs=w.tiles.map(t=>{const s=t.settlement,c=farmConditions(w,t.id);return s&&open[t.id]&&c.land&&!c.flooded&&c.temperaturePermille>0&&c.moisturePermille>0&&c.fertility>0&&s.settings.workerRationsPerDay>0?Math.max(0,Math.ceil(s.settings.farmRationsPerDay/s.settings.workerRationsPerDay)-t.population):0;});
