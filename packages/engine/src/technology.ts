...
export function validateTechnology(w:World):void {
 ...
 const visit=(id:string)=>{if(visiting.has(id))throw Error('Technology prerequisite cycle');if(visited.has(id))return;const d=definitions.get(id);if(!d)throw Error('Unknown technology prerequisite');const deps=d.prerequisites??[];if(new Set(deps).size!==deps.length)throw Error('Duplicate technology prerequisite');visiting.add(id);for(const dep of deps)visit(dep);visiting.delete(id);visited.add(id);};
 for(const id of definitions.keys())visit(id);
 for(const t of w.tiles){const r=t.research;if(!r)continue;const ids=new Set<string>();for(const p of r.projects){const d=definitions.get(p.technologyId);if(!d||ids.has(p.technologyId)||p.progress>d.workRequired||(p.completedTick!==undefined)!==(p.progress===d.workRequired)||p.completedTick!==undefined&&p.completedTick>w.tick)throw Error('Invalid research progress');if(p.progress>0&&(d.prerequisites??[]).some(id=>{const dep=r.projects.find(p=>p.technologyId===id);return dep?.completedTick===undefined||p.completedTick!==undefined&&dep.completedTick>=p.completedTick;}))throw Error('Research prerequisite history is inconsistent');ids.add(p.technologyId);}if(r.assignment&&!ids.has(r.assignment.technologyId))throw Error('Unknown research assignment');if(r.lastDay&&(r.lastDay.tick>w.tick||(r.lastDay.knowledgeWork!==undefined)!==(r.lastDay.knowledgeSource!==undefined)||(r.lastDay.workers+(r.lastDay.knowledgeWork??0)!==r.lastDay.progressAdded||r.lastDay.knowledgeWork!==undefined&&(r.lastDay.knowledgeWork>r.lastDay.workers||!tech.exchange||r.lastDay.knowledgeSource===undefined||!w.cells[t.id].neighbors.includes(r.lastDay.knowledgeSource)))))throw Error('Invalid research daily accounting');}
}
export function applyTechnology(w:World,op:Operation):void {
 ...
 if(op.kind==='technology-define'){if(!w.technology)throw Error('Activate technology first');if(op.expectedVersion!==w.technology.version)throw Error('Stale technology version');if(w.technology.definitions.some(d=>d.id===op.definition.id))throw Error('Technology definitions are immutable; choose a new ID');if(op.definition.prerequisites?.some(id=>!w.technology!.definitions.some(d=>d.id===id)))throw Error('Define prerequisites first');w.technology.definitions.push({...op.definition});w.technology.version++;return;}
...
export function prerequisitesMet(w:World,tileId:number,technologyId:string):boolean {
 const d=w.technology?.definitions.find(d=>d.id===technologyId);return !!d&&(d.prerequisites??[]).every(id=>w.tiles[tileId].research?.projects.some(p=>p.technologyId===id&&p.completedTick!==undefined&&p.completedTick<w.tick));
}
export function technologyPermille(w:World,id:number,effect:'farmBonusPermille'|'treatmentBonusPermille'):number {
 if(!w.technology?.enabled)return 1000;const completed=new Set(w.tiles[id].research?.projects.filter(p=>p.completedTick!==undefined&&p.completedTick<w.tick).map(p=>p.technologyId));return 1000+Math.min(1000,w.technology.definitions.filter(d=>completed.has(d.id)).reduce((n,d)=>n+(d[effect]??0),0));
}
export const farmTechnologyPermille=(w:World,id:number)=>technologyPermille(w,id,'farmBonusPermille');
export const treatmentTechnologyPermille=(w:World,id:number)=>technologyPermille(w,id,'treatmentBonusPermille');
 ...
 for(const t of w.tiles){
  if(w.technology.enabled&&t.settlement&&t.population&&t.elevationM>0){
   ...
   if(!r.paused&&(!r.assignment||r.assignment.automatic)){
    ...
    const definition=current??[...w.technology.definitions].sort((a,b)=>a.workRequired-b.workRequired||(a.id<b.id?-1:a.id>b.id?1:0)).find(d=>prerequisitesMet(w,t.id,d.id)&&!r.projects.some(p=>p.technologyId===d.id&&p.completedTick!==undefined));
  ...
  if(w.technology.enabled&&a&&p&&d){
   ...
   else if(!prerequisitesMet(w,t.id,d.id))reason='prerequisites';
   ...
   else{const v=away.get(t.id),ill=w.disease?.enabled?(t.health!.ill-(v?.ill??0)):0,available=Math.max(0,t.population-(v?.population??0)-ill-garrisonWorkers(w,t.id));const source=exchange&&t.communication?[...w.cells[t.id].neighbors].sort((a,b)=>a-b).find(id=>known[id].has(d.id)&&factionBorderOpen(w,t.id,id,'knowledge')):undefined;
