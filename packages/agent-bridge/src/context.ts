...
export const capabilities={
 ...
 mechanics:['One simulated day per tick','Integer water volumes in litres; 1 L/m² = 1 mm','Retain 20 mm, then move half excess to lower neighbors simultaneously','Ocean water is an accounted sink','Seasonal temperature, evaporation, vegetation response','Existing sediment travels with runoff'],
 unsupported:['New code or plugins','New rule primitives or custom fields','Soil erosion generating sediment','Trade, warfare, research, migration, or population dynamics','Artwork generation','Automatic model calls during ticks'],
...
export function buildContext(world:World,request:PromptRequest,history:PromptJob[]) {
 ...
 return {capabilities,mode:request.mode,scope:request.scope,allowedTileIds,
  ...
  tiles:ids.map(id=>({...world.tiles[id],areaM2:world.cells[id].areaM2,neighbors:world.cells[id].neighbors,standingWaterMm:world.tiles[id].waterL/world.cells[id].areaM2})),
