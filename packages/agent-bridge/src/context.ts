...
export const capabilities={
 version:4,operations:{
  'field-define':{meaning:'Create/update a WORLD-WIDE numeric property definition. Requires world scope. Version starts at 1, update increments by 1; bounds/default use max 3 decimal places. migration preserve rejects discarded values; clamp explicitly clamps all existing values. Max 32 properties. quantity stock explicitly enables conserved transfers and requires min 0; omitted quantity or index is a non-conserved measure.'},
 ...
 unsupported:['New code or plugins','Arbitrary expression operators, custom built-in physics, long-distance resource routes, or entity/building/technology systems','Freezing/melting water, steam, fire, radiation, or asteroid physics (temperature/elevation changes can approximate an event only)','Soil erosion generating sediment','Trade, warfare, research, migration, or population dynamics','Artwork generation','Automatic model calls during ticks'],
...
export function buildContext(world:World,request:PromptRequest,history:PromptJob[]) {
 ...
 return {capabilities,mode:request.mode,scope:request.scope,allowedTileIds,
  ...
  selectedTileId:tile.id,definitions:world.definitions,resourceLedger:world.resourceLedger,
