import {z} from 'zod';
export const ArtworkSlot=z.enum(['ocean','city','alpine','forest','meadow','dry']);
...
export const ArtworkPackSchema=z.object({
 ...
 images:z.array(z.object({slot:ArtworkSlot,hash}).strict()).min(1).max(6),
}).strict();
export const ArtworkImportSchema=z.object({format:z.literal('logos-artwork'),version:z.literal(1),pack:ArtworkPackSchema,images:z.array(z.object({hash,data:z.string().max(12*1024*1024)}).strict()).min(1).max(6)}).strict();
