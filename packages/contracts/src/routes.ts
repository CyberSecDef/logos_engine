...
export const RouteDayEntrySchema=z.object({routeId:id,from:tile,to:tile,fieldId:id,label:z.string().max(80),unit:z.string().max(24),amountMilli:z.number().int().min(0).max(1e12),status:z.enum(['moved','limited','paused','not-due','closed','submerged'])}).strict();
