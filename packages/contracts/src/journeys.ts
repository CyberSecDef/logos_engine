...
export const MilitarySchema=z.object({mission:z.enum(['assault','return','cancelled']).optional(),factionId:id,homeTileId:tile,reserveDays:z.number().int().min(0).max(3650)}).strict();
export const JourneySchema=z.object({military:MilitarySchema.optional(),health:HealthSchema.optional(),id,label:z.string().trim().min(1).max(80),path,daysPerHop:days,index:z.number().int().min(0).max(63),remainingDays:z.number().int().min(0).max(30),population,foodRations:food,cargo,departedTick:z.number().int().nonnegative(),shortageDays:z.number().int().min(0).max(3650),shortageIntervalDays:z.number().int().min(1).max(3650),lossPermille:z.number().int().min(0).max(1000)}).strict();
export const JourneyReportSchema=z.object({factionId:id.optional(),journeyId:id,label:z.string().max(80),origin:tile,tileId:tile,destination:tile,status:z.enum(['moving','battle-ready','arrived','closed','border-closed','military-access','submerged','no-settlement','population-capacity','food-capacity','cargo-capacity','no-travelers']),beforePopulation:population,afterPopulation:population,beforeFood:food,afterFood:food,consumed:population,unmet:population,losses:population}).strict();
...
export const journeyOperations=[
 ...
 z.object({kind:z.literal('army-depart'),tileId:tile,journeyId:id,label:z.string().trim().min(1).max(80),path,daysPerHop:days,population:population.min(1),foodRations:food}).strict(),
] as const;
