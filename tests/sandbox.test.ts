...
test('local provider sandbox hides engine files and confines writes to scratch',async t=>{
 ...
 try{await mkdir(home);await mkdir(work);
  const {stdout}=await promisify(execFile)('/usr/bin/bwrap',[...sandboxArgs('/usr/bin/sh',home,work),'--','/opt/claude','-c','test ! -w "$1" && test ! -w /usr && printf success > /tmp/world/check && printf isolated','--',resolve('package.json')],{env:{PATH:'/usr/bin:/bin'},timeout:10000});
