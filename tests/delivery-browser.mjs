...
try {
 ...
 const context=await browser.newContext({viewport:{width:1280,height:800},reducedMotion:'reduce'}),errors=[];
 const visit=async()=>{
 ...
 assert.ok(warm.every(a=>a.transferSize===0),'Repeat navigation must reuse hashed build assets');
 ...
 console.log(JSON.stringify({assets,cold,warm,totalJavaScriptBytes:total,notes:'Production server; isolated Chromium context, first and repeat navigation. Gzip sizes are estimates; HTTP uses identity encoding. No world writes or model calls.'},null,2));
